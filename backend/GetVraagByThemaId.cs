...
{
    public static class GetVraagByThemaId
    {
        [FunctionName("GetVraagByThemaId")]
        public static async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v1/vragen/{themaid}")] HttpRequest req,int themaid, ILogger log)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand sqlCommand = new SqlCommand())
                    {
                        ...
                        sqlCommand.CommandText = $"SELECT * FROM Vragen WHERE ThemaId = @themaid;"; //SQL command
